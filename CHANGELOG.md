...
| PR-8b | `3c96525`  | `RuntimeListener` v2: `TraceContext`, per-call timing, per-provider-call visibility, retry visibility, memory I/O hooks, sibling `GuardrailListener` |
...
| PR-22 | `62c2428`  | `config.yaml` for provider/model/timeout/format; `.env` now holds only API keys. `Provider` enum encodes default `base_url` + conventional API-key env var per provider (gemini / openai / ibm / ollama / omlx). |
