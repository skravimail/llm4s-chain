...
- `llm4s-core` owns the provider-neutral model: messages, content, tools, JSON schemas, response formats, usage, finish reasons, and capability metadata.
- `llm4s-runtime` executes chat requests and tool loops against a `ChatBackend`.
- `llm4s-openai-compat` is the first concrete backend and handles OpenAI-compatible HTTP/SSE wire formats.
- `llm4s-macros`, `llm4s-structured`, `llm4s-tools`, `llm4s-memory`, `llm4s-rag`, `llm4s-agentic`, `llm4s-mcp`, and `llm4s-guardrails` layer framework features around the core/runtime contracts.
- Legacy LangChain4j demo code remains outside the native module stack and is listed as post-roadmap cleanup.
